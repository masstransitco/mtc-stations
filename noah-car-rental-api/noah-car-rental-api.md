...
### 6. Get Vehicles with Future Bookings

**Endpoint:** `GET /api/vehicles/future-bookings`

**Description:** Get all vehicles with their future bookings. Future bookings are defined as bookings with pickup dates after today, excluding cancelled and completed bookings. This endpoint is useful for planning vehicle availability and managing upcoming reservations.
...
```json
{
      ...
      "futureBookings": [
        {
          ...
          "pickupAddress": "Hong Kong International Airport",
          "returnAddress": "Hong Kong International Airport",
          "pickupOption": "meet-and-greet",
          "returnOption": "drop-off",
          "rentalPrice": 5000,
          "rentalDeposit": 3000,
          "bookingDeposit": 1000,
  ...
  "summary": {
    "totalVehicles": 45,
    "vehiclesWithFutureBookings": 12,
    "totalFutureBookings": 18,
    "asOfDate": "2024-12-01"
  }
}
```

**Database Function:** `get_vehicles_with_future_bookings()`
- Located in: `sql/get_vehicles_with_future_bookings.sql`
- Filters bookings with `pickup_date > CURRENT_DATE`
- Excludes `'cancelled'` and `'completed'` bookings
- Returns full booking details with customer information
- Groups bookings by vehicle with aggregated arrays
- Calculates summary statistics

**Server Action:** `getVehiclesWithFutureBookings()`
- Located in: `lib/actions/vehicles.ts:836`
- Calls RPC function via Supabase client
- Returns structured response with error handling

**Use Cases:**
- Planning vehicle maintenance schedules
- Viewing upcoming reservations by vehicle
- Identifying available time slots between bookings
- Forecasting fleet utilization
- Managing vehicle allocation
...
### Get Vehicles with Future Bookings
```bash
curl https://noah.air.city/api/vehicles/future-bookings
```

### Filter Vehicles with Many Future Bookings
```bash
# Get vehicles that have 2 or more future bookings
curl -s https://noah.air.city/api/vehicles/future-bookings | \
  jq '.vehicles[] | select(.futureBookingsCount >= 2)'
```
