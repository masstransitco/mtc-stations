-- Fix latest_metered_carpark_occupancy materialized view with time filter
-- Problem: The CTE scans ALL snapshots without time filter, causing stale data
-- Solution: Add time filter to only look at recent data (last 1 hour)

-- Drop and recreate with time filter
...
WITH latest_spaces AS (
  ...
  WHERE is_valid = true
    AND ingested_at >= NOW() - INTERVAL '1 hour'  -- Time filter for performance
...
COMMENT ON MATERIALIZED VIEW latest_metered_carpark_occupancy IS
  'Materialized view of aggregated real-time occupancy data per metered carpark. Time-filtered for performance. Refreshed every 5 minutes by cron job.';
